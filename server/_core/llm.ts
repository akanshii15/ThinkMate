...
const convertMessagesToGemini = (messages: Message[]) => {
  ...
  for (const message of messages) {
    if (message.role === "system") {
      systemInstruction = ensureArray(message.content)
        .map(part => {
    ...
    const contentArray = ensureArray(message.content);
    for (const part of contentArray) {
      if (typeof part === "string") {
        ...
        // For now, we'll skip images as they require additional processing
        console.warn("Image content not yet supported in Gemini integration");
      }
    }
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  const { geminiMessages, systemInstruction } = convertMessagesToGemini(messages);
  ...
  if (normalizedResponseFormat?.type === "json_schema" || normalizedResponseFormat?.type === "json_object") {
    ...
    const jsonInstruction = normalizedResponseFormat.type === "json_schema"
      ? `Respond with valid JSON matching this schema: ${JSON.stringify(normalizedResponseFormat.json_schema.schema)}`
